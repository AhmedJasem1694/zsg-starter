    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="space-y-2">
          <Link to="/app/legal/library" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground/80 transition-colors">
            <ArrowLeft size={13} /> Back to library
          </Link>
          <h1 className="text-2xl font-bold text-foreground">{name}</h1>
          <p className="text-sm text-muted-foreground">Everything Zane knows about how you deal with this vendor.</p>
        </div>

        {isLoading && <div className="text-center py-16 text-muted-foreground text-sm">Loading vendor intelligence…</div>}
          ...
          <>
            ...
            <section className="card shadow-sm p-5 sm:p-6 space-y-4">
              ...
              {profile ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="rounded-lg border border-border px-4 py-3">
                      ...
                      <div className="text-[11px] text-muted-foreground mt-0.5">Contracts</div>
                    </div>
                    <div className="rounded-lg border border-border px-4 py-3">
                      <div className="text-lg font-semibold text-foreground">{profile.totalMoves}</div>
                      <div className="text-[11px] text-muted-foreground mt-0.5">Captured moves</div>
                    </div>
                    <div className="rounded-lg border border-border px-4 py-3">
                      <div className="text-lg font-semibold text-foreground">{profile.avgRoundsToClose ?? "-"}</div>
                      <div className="text-[11px] text-muted-foreground mt-0.5">Avg rounds to close</div>
                    </div>
                  </div>
                  <p className="text-sm text-foreground/70">Typically {profile.typicalMovement}.</p>
                  {profile.alwaysPushOn.length > 0 && (
                    <div>
                      <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1.5">Consistently pushes on</div>
                  ...
                  {profile.neverConcede.length > 0 && (
                    <div>
                      <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1.5">Rarely concedes</div>
                      <div className="flex flex-wrap gap-1.5">
                ...
                <p className="text-xs text-muted-foreground">No negotiation history captured yet. This builds as contracts with this vendor are negotiated over email.</p>
              )}
            </section>
                ...
                <div className="divide-y divide-border/50">
                  {decisions.map((d, i) => (
                    <div key={i} className="py-3 first:pt-0 last:pb-0">
                      ...
                      {d.reason && <div className="text-xs text-muted-foreground mt-1 leading-relaxed">Reason: {d.reason}</div>}
                      <div className="text-[11px] text-muted-foreground mt-1">{d.contractName}{d.contractName && formatDate(d.created) ? " · " : ""}{formatDate(d.created)}</div>
                    </div>
                  ))}
                </div>
              </section>
            ...
            <section className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-foreground">Documents <span className="font-normal text-muted-foreground">({docs.length})</span></h2>
              </div>
              {docs.length === 0 ? (
                <p className="text-xs text-muted-foreground">No documents with this vendor yet.</p>
              ) : (
                <div className="space-y-5">
                  {docGroups.map((g) => (
                    <div key={g.label} className="space-y-1.5">
                      <div className="text-[11px] uppercase tracking-wider text-muted-foreground">{g.label} ({g.docs.length})</div>
